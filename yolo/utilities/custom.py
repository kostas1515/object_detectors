...
class EQLoss(nn.Module):
    ...
    def __init__(self, loss_fcn,img_freq, gamma=1.5, alpha=0.25, threshold=0.0045):
        ...
        self.reduction = loss_fcn.reduction
        self.loss_fcn.reduction = 'none'  # required to apply FL to each element
        freq_i = img_freq / img_freq.sum()
        self.eq_mask = (freq_i < threshold) +0.0


    def forward(self, pred, true):
        bs = pred.shape[0]
        weights = torch.stack([self.eq_mask for k in range(bs)],axis=0)
        weights += true
