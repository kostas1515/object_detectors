...
def maskrcnn_inference(x, labels):
    ...
    num_masks = x.shape[0]
    boxes_per_image = [len(ll) for ll in labels]
...
def maskrcnn_loss(mask_logits, proposals, gt_masks, gt_labels, mask_matched_idxs):
    ...
    labels = [ll[idxs] for ll, idxs in zip(gt_labels, mask_matched_idxs)]
    mask_targets = [
...
class RoIHeads(torch.nn.Module):
    ...
    def assign_targets_to_proposals(self, proposals, gt_boxes, gt_labels):
        ...
        for proposals_in_image, gt_boxes_in_image, gt_labels_in_image in zip(proposals, gt_boxes, gt_labels):
            ...
            labels_in_image[bg_inds] = torch.tensor(0,device='cuda')

            # Label ignore proposals (between low and high thresholds)
            ignore_inds = matched_idxs_in_image == self.proposal_matcher.BETWEEN_THRESHOLDS
            labels_in_image[ignore_inds] = torch.tensor(-1,device='cuda')  # -1 is ignored by sampler
