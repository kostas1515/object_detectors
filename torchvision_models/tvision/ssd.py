...
class SSD(nn.Module):
    ...
    def __init__(self, backbone: nn.Module, anchor_generator: DefaultBoxGenerator,
                 size: Tuple[int, int], num_classes: int,tfidf=None,
    ...
    def compute_loss(self, targets: List[Dict[str, Tensor]], head_outputs: Dict[str, Tensor], anchors: List[Tensor],
        ...
        cls_loss = F.cross_entropy(
            self.tfidf*cls_logits.view(-1, num_classes),
            cls_targets.view(-1),
            reduction='none'
...
def ssd300_vgg16(pretrained: bool = False, progress: bool = True, num_classes: int = 91,
                 pretrained_backbone: bool = True, trainable_backbone_layers: Optional[int] = None,tfidf=None, **kwargs: Any):
    ...
    model = SSD(backbone, anchor_generator, (300, 300), num_classes,tfidf, **kwargs)
    if pretrained:
